...
class BiLSTMSentiment(nn.Module):
    def __init__(self, init_embed,
        ...
        self.lstm = LSTM(input_size=self.embed.embedding_dim, hidden_size=hidden_dim, num_layers=num_layers,
                         bidirectional=True)
...
def train(dataset_name, split_dataset_name,number_class, text_name='text'):
    ...
    embed = StaticEmbedding(vocab, model_dir_or_name='en-glove-840b-300d', requires_grad=True)
    model = BiLSTMSentiment(init_embed=embed, num_classes=opt.num_classes, hidden_dim=opt.hidden_dim,
                            num_layers=opt.num_layers, nfc=opt.nfc)
