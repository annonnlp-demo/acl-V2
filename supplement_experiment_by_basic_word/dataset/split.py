...
def split_selected_dataset_to_ttd(sorted_split_dataset_list,text_name):
    ...
    return convert_to_small(train,dataset_name,text_name), convert_to_small(dev,dataset_name,text_name), convert_to_small(test,dataset_name,text_name)
...
def get_sorted_index(label_number, split_raw_dataset, text_name='text'):
    results = []

    for index in range(label_number):
        tem = get_sorted_index_(split_raw_dataset[index], text_name)
        results.append(tem)

    return results


def get_sorted_index_(s_dataset, text_name='text'):
    dic = {}

    from tqdm import tqdm
    for index in tqdm(range(len(s_dataset))):
        text = s_dataset[index][text_name]
        dic[index] = get_basic_word_po(text)
...
def get_basic_word_po(text):
    value=text
    value_list = value.split(' ')
    _number = len(value_list)
    _other=0
...
sorted_index_list = get_sorted_index(label_number, split_raw_dataset_list, text_name=text_name)
